---
title: "Analysis of Variance - demographic factors"
...
```{r, echo=TRUE}
data <- read.csv("Spring_All_Analysis_Augmented.csv")
summary(data)
...
# compositescore[data$Group==1 & data$Session==1]
# compositescore[data$Group==2 & data$Session==1]
# compositescore[data$Group==1 & data$Session==2]
# compositescore[data$Group==2 & data$Session==2]
# 
# confcompscore[data$Group==1 & data$Session==1]
# confcompscore[data$Group==2 & data$Session==1]
# confcompscore[data$Group==1 & data$Session==2]
# confcompscore[data$Group==2 & data$Session==2]

dgdata <- read.csv("Spring_Demographics_Everything.csv")
summary(dgdata)
```
...
### Create subset of above data that can be joined - intersect Session1 and Session2 IDs

```{r, echo=TRUE}
mndx <- match(labels(session1totalIDCorrect), labels(session2totalIDCorrect))
# match finds the positions of Session1's IDs in Session2
# IDs in Session1 not in Session2 appear as NA
session1totalIDCorrect
session2totalIDCorrect
mndx
session1totalIDCorrect[!is.na(mndx)] # remove IDs not in Session2
session2totalIDCorrect[mndx[!is.na(mndx)]] # IDs in Session2 not in Session1 don't show up in mndx

filter1 <- !is.na(mndx)
filter2 <- mndx[!is.na(mndx)]
msession1totalIDCorrect <- session1totalIDCorrect[filter1]
msession2totalIDCorrect <- session2totalIDCorrect[filter2]
msession1totalIDTime <- session1totalIDTime[filter1]
msession2totalIDTime <- session2totalIDTime[filter2]
msession1totalIDConf <- session1totalIDConf[filter1]
msession2totalIDConf <- session2totalIDConf[filter2]
msession1IDGroup <- session1IDGroup[filter1]
msession2IDGroup <- session2IDGroup[filter2]
...
```{r, echo=TRUE}
t.test(msession1totalIDCorrect[msession1IDGroup == 1], msession2totalIDCorrect[msession2IDGroup == 1], paired=T)
t.test(session1totalIDCorrect[session1IDGroup == 1], session2totalIDCorrect[session2IDGroup == 1])
wilcox.test(msession1totalIDCorrect[msession1IDGroup == 1], msession2totalIDCorrect[msession2IDGroup == 1], paired=T)
wilcox.test(session1totalIDCorrect[session1IDGroup == 1], session2totalIDCorrect[session2IDGroup == 1])

t.test(msession1totalIDCorrect[msession1IDGroup == 2], msession2totalIDCorrect[msession2IDGroup == 2], paired=T)
t.test(session1totalIDCorrect[session1IDGroup == 2], session2totalIDCorrect[session2IDGroup == 2])
wilcox.test(msession1totalIDCorrect[msession1IDGroup == 2], msession2totalIDCorrect[msession2IDGroup == 2], paired=T)
wilcox.test(session1totalIDCorrect[session1IDGroup == 2], session2totalIDCorrect[session2IDGroup == 2])

group1totaldiff <- msession2totalIDCorrect[msession2IDGroup == 1] - msession1totalIDCorrect[msession1IDGroup == 1]
group2totaldiff <- msession2totalIDCorrect[msession2IDGroup == 2] - msession1totalIDCorrect[msession1IDGroup == 2]
...
```
Shapiro-Wilk test indicates that the distributions are different from normal (p<0.01 for both sessions) - this has changed from the previous measure of correctness, which was normally distributed.  
As a result, some of the linear model results below may not be valid.
...
dgndx is the row in dgdata whose ID corresponds with msession1IDGroup's ID
(use msession1IDGroup rather than session1IDGroup as we will later join Session1 and Session2 into common table)
```{r, echo=TRUE}
dgndx <- match(names(msession1IDGroup), dgdata$ID)
...
```{r, echo=TRUE}
Group <- as.factor(msession1IDGroup)
IDdata <- data.frame(Group, msession1totalIDCorrect, msession2totalIDCorrect, msession1totalIDTime, msession2totalIDTime, msession1totalIDConf, msession2totalIDConf, OSSExp, ProExp, ModelExp, CSBg, LawBg, DStructExp, EnglProf)
names(IDdata)[names(IDdata) == "msession1totalIDCorrect"] <- "session1totalIDCorrect"
names(IDdata)[names(IDdata) == "msession2totalIDCorrect"] <- "session2totalIDCorrect"
names(IDdata)[names(IDdata) == "msession1totalIDTime"] <- "session1totalIDTime"
names(IDdata)[names(IDdata) == "msession2totalIDTime"] <- "session2totalIDTime"
names(IDdata)[names(IDdata) == "msession1totalIDConf"] <- "session1totalIDConf"
names(IDdata)[names(IDdata) == "msession2totalIDConf"] <- "session2totalIDConf"
```
