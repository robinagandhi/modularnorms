...
```{r, echo=TRUE}
data <- read.csv("All_Time_Analysis_Everything.csv")
summary(data)
...
dgdata <- read.csv("Demographics_Everything.csv")
summary(dgdata)
```
## Overwriting notes
...
To analyze percentage-based score, keep everything as is.

## Overwrite answers with percentage based answers
Comment out this block to get actual student answer accuracy
```{r, echo=TRUE}
hipaaraw <- read.csv("Session2-HIPAAGroup2-FINAL_Calculations.csv")
agplraw <- read.csv("Session2-AGPLGroup2-FINAL_Calculations.csv")
answers <- data$Answer
treatsess <- data$Group==2 & data$Session==2
matchraw <- match(data$ID[treatsess & data$QType=="HIPAA" & data$QID==1], hipaaraw$QID_ID)
answers[treatsess & data$QType=="HIPAA" & data$QID==1] <- hipaaraw$QID1_AdjustedCheck[matchraw]
answers[treatsess & data$QType=="HIPAA" & data$QID==2] <- hipaaraw$QID2_AdjustedCheck[matchraw]
answers[treatsess & data$QType=="HIPAA" & data$QID==3] <- hipaaraw$QID3_AdjustedCheck[matchraw]
matchraw <- match(data$ID[treatsess & data$QType=="AGPL" & data$QID==1], agplraw$QID_ID)
answers[treatsess & data$QType=="AGPL" & data$QID==1] <- agplraw$QID1_AdjustedCheck[matchraw]
answers[treatsess & data$QType=="AGPL" & data$QID==2] <- agplraw$QID2_AdjustedCheck[matchraw]
compositescore[treatsess] <- ifelse(answers[treatsess] == "Correct", 4, ifelse(answers[treatsess] == "Wrong", -4, ifelse(answers[treatsess] == "NMI", -4, NA)))
confcompscore[treatsess] <- ifelse(data$Confidence[treatsess] == "Very", compositescore[treatsess], ifelse(data$Confidence[treatsess] == "Semi", compositescore[treatsess]/2, compositescore[treatsess]/4))
